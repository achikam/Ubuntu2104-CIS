...
**This role is based on 18.04 migration. The tasks are correct in content, but have to be re-sorted to fit the 21.04 Benchmark. Contribution welcome.**

Configure Ubuntu 21.04 machine to be CIS compliant. Level 1 and 2 findings will be corrected by default.
...
```
- src: https://github.com/achikam/Ubuntu2104-CIS.git
```
...
(i.e. the playbook .yml and the directory `ubuntu2104_cis` should be next to each other),
...
**ubuntu2104cis_notauto**: Run CIS checks that we typically do NOT want to automate due to the high probability of breaking the system (Default: false)

**ubuntu2104cis_section1**: CIS - General Settings (Section 1) (Default: true)

**ubuntu2104cis_section2**: CIS - Services settings (Section 2) (Default: true)

**ubuntu2104cis_section3**: CIS - Network settings (Section 3) (Default: true)

**ubuntu2104cis_section4**: CIS - Logging and Auditing settings (Section 4) (Default: true)

**ubuntu2104cis_section5**: CIS - Access, Authentication and Authorization settings (Section 5) (Default: true)

**ubuntu2104cis_section6**: CIS - System Maintenance settings (Section 6) (Default: true)  

### Disable all selinux functions
`ubuntu2104cis_selinux_disable: false`

### Service variables
####These control whether a server should or should not be allowed to continue to run these services

```
ubuntu2104cis_avahi_server: false  
ubuntu2104cis_cups_server: false  
ubuntu2104cis_dhcp_server: false  
ubuntu2104cis_ldap_server: false  
ubuntu2104cis_telnet_server: false  
ubuntu2104cis_nfs_server: false  
ubuntu2104cis_rpc_server: false  
ubuntu2104cis_ntalk_server: false  
ubuntu2104cis_rsyncd_server: false  
ubuntu2104cis_tftp_server: false  
ubuntu2104cis_rsh_server: false  
ubuntu2104cis_nis_server: false  
ubuntu2104cis_snmp_server: false  
ubuntu2104cis_squid_server: false  
ubuntu2104cis_smb_server: false  
ubuntu2104cis_dovecot_server: false  
ubuntu2104cis_httpd_server: false  
ubuntu2104cis_vsftpd_server: false  
ubuntu2104cis_named_server: false  
ubuntu2104cis_allow_autofs: false
```  

### Designate server as a Mail server
`ubuntu2104cis_is_mail_server: false`


####System network parameters (host only OR host and router)
`ubuntu2104cis_is_router: false`  


####IPv6 required
`ubuntu2104cis_ipv6_required: true`  


### AIDE
`ubuntu2104cis_config_aide: true`

#### AIDE cron settings
```
ubuntu2104cis_aide_cron:
  cron_user: root
...
`ubuntu2104cis_xwindows_required: no`


### Client application requirements
```
ubuntu2104cis_openldap_clients_required: false
ubuntu2104cis_telnet_required: false
ubuntu2104cis_talk_required: false  
ubuntu2104cis_rsh_required: false
ubuntu2104cis_ypbind_required: false
ubuntu2104cis_rpc_required: false
```

### Time Synchronization
```
ubuntu2104cis_time_synchronization: chrony
ubuntu2104cis_time_Synchronization: ntp

ubuntu2104cis_time_synchronization_servers:
  - uri: "0.pool.ntp.org"
    config: "minpoll 8"
  - uri: "1.pool.ntp.org"
    config: "minpoll 8"
...
```yaml
ubuntu2104cis_rule_1_5_3: true
```
To use other than random password:
```yaml
ubuntu2104cis_root_password: 'new password'
```

```
ubuntu2104cis_firewall: firewalld
ubuntu2104cis_firewall: iptables
```

### 5.3.1 | PATCH | Ensure password creation requirements are configured
```
ubuntu2104cis_pwquality:
  - key: 'minlen'
    value: '14'
  - key: 'dcredit'
    value: '-1'
